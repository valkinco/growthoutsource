 ...
 * This is the TRUE intent used to resolve a Challenge — always accurate, because
 * the resolution itself must be fair. What the player sees before choosing is a
 * separate, deliberately imperfect read — see perceivedRivalIntent below.
 */
...
export function perceivedRivalIntent(state: GameState): { move: ChallengeMove; confident: boolean } {
  ...
  const visionScore = state.traits.vision + (has(state, 'negotiation') ? 2 : 0);
  const accuracy = Math.min(0.95, 0.35 + visionScore * 0.12);
  const rng = new Rng(`${state.seed}-intent-${state.turn}-${state.founder.q}-${state.founder.r}`);
...
export function rivalTurn(state: GameState, rng: Rng): GameState {
  ...
  if (primary.level === 'outpost' && next.rivalMomentum >= 5) {
    primary.level = 'town';
    next.rivalMomentum -= 5;
  } else if (primary.level === 'town' && next.rivalMomentum >= 8) {
    primary.level = 'city';
    next.rivalMomentum -= 8;
  } else if (rivalSettlements.length < 2 && next.rivalMomentum >= 6 && rng.chance(0.4)) {
